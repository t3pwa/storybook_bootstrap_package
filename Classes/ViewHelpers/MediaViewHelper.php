...
use Faeb\Videoprocessing\Service\VideoService;
...
class MediaViewHelper extends AbstractTagBasedViewHelper
{
    ...
    {
        parent::__construct();
        $this->videoService = GeneralUtility::makeInstance(VideoService::class);
...
//        var_dump ( $src = (string)$this->arguments['src'] );
//        var_dump ( $this->arguments['file'] );

        // exit;

        $treatIdAsReference =  $this->arguments['treatIdAsReference'];
        $src =  $this->arguments['src'];
        if (($src === '' && $this->arguments['file'] === null) || ($src !== '' && $this->arguments['file'] !== null)) {
...
//        $fileRenderer = GeneralUtility::makeInstance(RendererRegistry::class)->getRenderer($file);
        ...
        if ($this->arguments['treatIdAsReference']) {
            //            $src = $this->arguments['id'];
        }



        $video = $this->videoService->getVideo($src, $this->arguments['file'], (bool)$this->arguments['treatIdAsReference']);

        // var_dump("video", $video); exit;


        $renderer = GeneralUtility::makeInstance(RendererRegistry::class)->getRenderer($video);
        $configuration = [];
        // $additionalConfig = array_merge_recursive($this->arguments, $additionalConfig);
        return $renderer->render($video, 0, 0, $configuration);

        //$fileRenderer = GeneralUtility::makeInstance(RendererRegistry::class)->getRenderer($src);
//        return $fileRenderer = GeneralUtility::makeInstance(\Faeb\Videoprocessing\Rendering\VideoTagRenderer::class)->render($src);
        ...
        */
        ...
        return $fileRenderer->render($src, $width, $height, $additionalConfig);
        */
    }
